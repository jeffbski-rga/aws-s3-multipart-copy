...
const { createDeps, copyObjectMultipart } = require("../../src/copy-object-multipart");
...
describe("AWS S3 multipart copy client unit tests", function () {
  ...
  describe("Testing createDeps function", function () {
    it("Should pass when given valid s3 and logger objects", function () {
      s3 = new AWS.S3();
      const deps = createDeps(s3, logger);
      should(deps.log).be.an.Object();
      should(deps.s3CreateMultipartUpload).be.a.Function();
      should(deps.s3UploadPartCopy).be.a.Function();
      should(deps.s3AbortMultipartUpload).be.a.Function();
      should(deps.s3ListParts).be.a.Function();
      should(deps.s3CompleteMultipartUpload).be.a.Function();
    });
  });
  ...
  describe("Testing copyObjectMultipart", function () {
    ...
    it("Should succeed with all variables passed", function () {
      ...
      return copyObjectMultipart(deps, testData.full_request_options, testData.request_context).then(() => {
        should(loggerInfoSpy.callCount).equal(5);
        should(loggerInfoSpy.args[0][0]).eql({
          msg: "multipart copy initiated successfully: " + JSON.stringify({ UploadId: "1a2b3c4d" }),
    ...
    it("Should succeed with all mandatory variables passed", function () {
      ...
      return copyObjectMultipart(deps, testData.partial_request_options, testData.request_context).then(() => {
        should(loggerInfoSpy.callCount).equal(5);
        should(loggerInfoSpy.args[0][0]).eql({
          msg: "multipart copy initiated successfully: " + JSON.stringify({ UploadId: "1a2b3c4d" }),
          context: "request_context",
    ...
    it("Should url-encode CopySource key", function () {
      ...
      return copyObjectMultipart(deps, partial_request_options, testData.request_context).then(() => {
    ...
    it("Should succeed with all variables passed and object_size is smaller then copy_part_size_bytes", function () {
      ...
      return copyObjectMultipart(deps, full_request_options, testData.request_context).then(() => {
    ...
    it("Should fail due to createMultipartUpload error and not call abortMultipartCopy", function () {
      ...
      return copyObjectMultipart(deps, testData.partial_request_options, testData.request_context)
        .then(() => {
          throw new Error("copyObjectMultipart resolved when an error should have been rejected");
        })
        .catch((err) => {
    ...
    it("Should call abortMultipartCopy upon error from uploadPartCopy error and succeed", function () {
      ...
      return copyObjectMultipart(deps, testData.full_request_options, testData.request_context)
        .then(() => {
          throw new Error("copyObjectMultipart resolved when an error should have been rejected");
        })
        .catch((err) => {
          ...
          should(loggerInfoSpy.args[1][0]).eql({
            msg: "multipart copy aborted successfully: " + JSON.stringify({ Parts: [] }),
            context: "request_context",
          });
          should(loggerErrorSpy.callCount).equal(3);
    ...
    it("Should call abortMultipartCopy upon error from completeMultipartUpload and a list of parts returned from listParts", async function () {
      ...
      return copyObjectMultipart(deps, testData.full_request_options, testData.request_context)
        ...
        .catch((err) => {
          ...
          should(loggerErrorSpy.args[1][0]).eql("test_error");
          should(loggerErrorSpy.args[2][0]).eql({
